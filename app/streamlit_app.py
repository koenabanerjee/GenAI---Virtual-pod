...
def _start_run(requirements: str, project_name: str, original_input_path: Path | None = None) -> None:
...
with left:
    # offer an RFI/requirements upload in addition to free-text input
    rfi_file = st.file_uploader("Upload RFI / requirements file", type=["txt", "md", "pdf"])
    if rfi_file is not None:
        tmp_dir = Path(tempfile.mkdtemp())
        rfi_path = tmp_dir / rfi_file.name
        with open(rfi_path, "wb") as f:
            f.write(rfi_file.getbuffer())
        st.session_state.rfi_path = rfi_path
        try:
            from virtual_dev_pod.rfi import load_rfi

            parsed = load_rfi(rfi_path)
            st.session_state.rfi_requirements = parsed
            st.success(f"RFI parsed successfully from {rfi_file.name}")
        except Exception as e:
            st.error(f"Failed to parse RFI: {e}")
    # display name of the currently stored RFI if available
    if st.session_state.get("rfi_path"):
        st.write("Uploaded RFI:", st.session_state.rfi_path.name)

    if st.button("Clear RFI"):
        st.session_state.pop("rfi_path", None)
        st.session_state.pop("rfi_requirements", None)
        st.experimental_rerun()

    project_name = st.text_input("Project Name", value="virtual-development-pod-demo")
    requirements_text = st.session_state.get("rfi_requirements", "")
    ...
    if st.button("Run End-to-End SDLC", type="primary", disabled=run_button_disabled):
        if st.session_state.pod is None:
            ...
            orig = st.session_state.get("rfi_path")
            _start_run(requirements, project_name, original_input_path=orig)
            st.rerun()
...
with right:
    ...
    if st.session_state.run_result is not None:
        ...
        if hasattr(run_result, "original_input") and run_result.original_input:
            st.write("Original input file saved:", run_result.original_input)
...
# Display comprehensive SDLC stage outputs
if st.session_state.run_result is not None:
    run_result = st.session_state.run_result
    st.header("📊 SDLC Stage Outputs")
    
    # Create tabs for each stage
    tab_analysis, tab_ui, tab_design, tab_dev, tab_tests, tab_management = st.tabs([
        "📋 Analysis", 
        "🎨 UI Design", 
        "📐 Design", 
        "💻 Development", 
    ...
    # Analysis Stage - User Stories
    with tab_analysis:
        st.subheader("User Stories (Analysis)")
        user_stories_file = run_result.run_dir / "analysis" / "user_stories.md"
        display_user_stories(run_result.user_stories, user_stories_file)
    
    # UI Design Stage
    with tab_ui:
        st.subheader("UI Design Artifacts (UI Designer Agent)")
        if hasattr(run_result, "ui_artifacts"):
            display_ui_mockups(run_result.ui_artifacts, run_result.run_dir)
        else:
            st.info("No UI artifacts available.")
    
    # Design Stage
    with tab_design:
        st.subheader("Design Specifications (Design Agent)")
        display_design_specifications(run_result.design_artifacts)
    
    # Development Stage
    with tab_dev:
        st.subheader("Generated Source Code (Developer Agent)")
        display_generated_code(run_result.code_artifacts)
    
    # Testing Stage
    with tab_tests:
        st.subheader("Test Artifacts (Testing Agent)")
        display_generated_tests(run_result.test_artifacts)
        
        st.divider()
        st.subheader("Test Execution Results")
        if run_result.test_execution:
            display_test_execution_report(run_result.test_execution)
        else:
            st.info("Tests were not executed. Check VDP_EXECUTE_TESTS setting.")
    
    # Management Stage
    with tab_management:
        st.subheader("SDLC Run Summary (Product Manager)")
        display_run_summary(run_result)
