...
class UIPageArtifact:
    ...
    def to_dict(self) -> dict[str, Any]:
        payload = asdict(self)
        if self.mockup_path:
            payload["mockup_path"] = str(self.mockup_path)
        return payload
...
class RunResult:
    ...
    ui_artifacts: list[UIPageArtifact] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "ui_artifacts": [u.to_dict() for u in self.ui_artifacts],
            "original_input": str(self.original_input) if self.original_input else None,
