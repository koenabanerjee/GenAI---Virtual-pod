...
class VirtualDevelopmentPod:
    ...
    def run_sdlc(
        ...
        *,
        ...
        # if an original input file was provided, make a copy into artifacts
        if original_input_path is not None:
            dest_dir = analysis_dir / "original_input"
            dest_dir.mkdir(parents=True, exist_ok=True)
            try:
                copied = dest_dir / original_input_path.name
                # shamelessly use pathlib's copy
                from shutil import copy2

                copy2(original_input_path, copied)
                run_result.original_input = copied
            except Exception:
                # ignore copy failure but log event
                self._emit_event(
                    run_result,
                    event_callback,
                    agent="Orchestrator",
                    stage="setup",
                    status="error",
                    message="failed to copy original_input file",
                )
        ...
        # UI Designer stage
        run_result.stage_status["ui_design"] = "in_progress"
        self._emit_event(
            run_result,
            event_callback,
            ...
            stage="ui_design",
            status="in_progress",
            message="Generating UI mockups and wireframes from user stories.",
        )
        ui_dir = run_dir / "ui"
        ui_artifacts = self.ui_designer.generate_ui_artifacts(
            stories, output_dir=ui_dir
        )
        # convert to UIPageArtifact objects for run result
        from virtual_dev_pod.models import UIPageArtifact

        run_result.ui_artifacts = []
        for a in ui_artifacts:
            mock_path = ui_dir / "mockups" / f"{slugify(a.page_name)}.html"
            run_result.ui_artifacts.append(
                UIPageArtifact(
        ...
        run_result.stage_status["ui_design"] = "completed"
        self._emit_event(
            run_result,
            event_callback,
            agent="UI Designer Agent",
            stage="ui_design",
            status="completed",
            message=f"Generated {len(ui_artifacts)} UI mockups.",
        )
