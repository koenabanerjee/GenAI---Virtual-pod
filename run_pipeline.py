...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Path to an RFI/requirements document (txt, md, pdf). Will be parsed and saved as original_input.",
    )
    parser.add_argument(
        "--project-name",
        type=str,
    ...
    if args.rfi_file.strip():
        from virtual_dev_pod.rfi import load_rfi

        rfi_path = Path(args.rfi_file)
        if not rfi_path.exists():
            raise FileNotFoundError(f"RFI file not found: {rfi_path}")
        requirements = load_rfi(rfi_path)
        original_input = rfi_path
    elif args.requirements_text.strip():
